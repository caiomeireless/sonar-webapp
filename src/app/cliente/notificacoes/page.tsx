// Central de Notificacoes — cliente. Server Component.
//
// Mesma estrutura visual da pagina da equipe, mas com categorias do canal
// CLIENTE e filtro por destinatario (email do usuario logado ou do ?eu= em
// modo visualizacao).
...
export default async function NotificacoesClientePage({ searchParams }: Props) {
  ...
  const todas = await listarNotificacoesCliente(emailEfetivo);
  ...
  const qsEu = ehVisualizacao && emailEfetivo
    ...
    : "";
  const linkComEu = (base: string, extra?: string) => {
    if (!extra) return base + qsEu;
    return qsEu ? `${base}?eu=${encodeURIComponent(emailEfetivo ?? "")}&${extra}` : `${base}?${extra}`;
  };
  ...
  return (
    <main className="mx-auto max-w-[1200px] px-6 py-10 sm:px-10">
      ...
      <header className="title-shield mb-8 text-center">
        ...
        <p className="font-mono text-[12px] uppercase tracking-[0.28em] text-[var(--color-gold)]">
          O Que Aconteceu na Sua Carteira
        </p>
        ...
        <p className="mx-auto mt-3 max-w-[640px] font-mono text-[13px] text-[var(--color-ivory-88)]">
          Novos bens encontrados, recuperações efetivadas, andamentos
          processuais e mensagens do escritório.
        </p>
      </header>
      ...
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
        <div className="flex flex-wrap items-center gap-2">
          <ChipFiltro
            ...
            href={linkComEu("/cliente/notificacoes")}
            label="Todas"
          ...
          <ChipFiltro
            ...
            href={linkComEu("/cliente/notificacoes", "status=nao-lidas")}
            label="Não Lidas"
          ...
          {CATEGORIAS_CLIENTE.map((c) => {
            ...
            return (
              <ChipFiltro
                ...
                href={linkComEu("/cliente/notificacoes", `categoria=${c.chave}`)}
                label={c.rotulo}
                contador={total}
                cor={c.cor}
              />
        ...
        {naoLidasNoTotal > 0 ? (
          <form action={marcarTodasComoLidasForm}>
            <input type="hidden" name="email" value={emailEfetivo ?? ""} />
            <button
              type="submit"
              className="
      ...
      {filtradas.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {filtradas.map((n) => {
            const cfg = configCategoria("cliente", n.categoria as NotificacaoCategoriaCliente);
            const cor = cfg?.cor ?? "var(--color-ivory-66)";
            const Icone = cfg?.icone ?? Bell;
            const destaca = n.categoria === "recuperacao-efetivada"
              || n.categoria === "patrimonio-encontrado";
            return (
              <Link
                key={n.id}
                href={`/cliente/notificacoes/${n.id}${qsEu}`}
                className={`
              ...
              >
                ...
                {destaca && n.referencias?.valor_brl ? (
                  <div
                  ...
                  >
                    <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-[var(--color-ivory-66)]">
                      Valor estimado
                    </div>
                    <div
                      className="mt-0.5 font-mono text-xl font-medium"
                      style={{ color: cor }}
                    >
