...
// Server Actions do módulo Intimação/Citação no Endereço — resultado da
// tentativa de intimar ou citar o devedor no endereço encontrado. SÓ EQUIPE.
//
// ARQUITETURA DO UPLOAD (mesma dos módulos Imóveis e Mandados): o arquivo
// NÃO viaja pela server action — a Vercel corta corpo de request em
// ~4.5 MB (413), o que tornaria a promessa de "até 20 MB" mentira em
// produção. Fluxo em 2 passos:
//   1) criarUploadIntimacaoUrl: valida MIME + emite URL assinada de upload
//      no bucket privado `imoveis-pesquisas` (COMPARTILHADO com os módulos
//      de Imóveis e de Mandados); o NAVEGADOR sobe direto (PUT).
//   2) registrarIntimacaoEndereco: recebe só os PATHS + campos, confere que
//      os ARs existem no bucket (e que ninguém reusa path de outro
//      registro — de intimação, de mandado OU de pesquisa de imóvel, já
//      que o bucket é o mesmo) e grava a intimação.
...
import type { ResultadoIntimacao } from "@/lib/enderecos-intimacoes";

const TABELA = "enderecos_intimacoes";
const TABELA_MANDADOS = "enderecos_mandados";
const TABELA_IMOVEIS = "imoveis_pesquisas";
const MAX_ANEXOS = 4;

const RESULTADOS_VALIDOS: ResultadoIntimacao[] = [
  "aguardando",
  "exito",
  "sem_exito",
];
...
// Sufixo "ar" amarra o path a ESTE módulo — os módulos de Imóveis
// ("print"/"matricula") e de Mandados ("mandado") usam outros sufixos,
// então um path nunca vale em dois módulos ao mesmo tempo.
const RE_PATH_ANEXO = /^dev-(\d+)\/\d{10,}-[a-z0-9]{4,12}-ar\.(pdf|jpg|png)$/;
...
// ---------------------------------------------------------------------------
// Passo 1 — URL assinada pro navegador subir o AR direto no bucket.
// ---------------------------------------------------------------------------
export async function criarUploadIntimacaoUrl(
  devedorId: number,
  contentType: string,
  ...
  try {
    ...
    const path = `dev-${devedorId}/${Date.now()}-${Math.random().toString(36).slice(2, 8)}-ar.${ext}`;
...
// ---------------------------------------------------------------------------
// Passo 2 — registra a intimação apontando pros ARs já subidos.
// ---------------------------------------------------------------------------
...
function lerAnexos(raw: string, devedorId: number): AnexoEntrada[] | Erro {
  ...
  if (brutos.length > MAX_ANEXOS) {
    return { erro: `No máximo ${MAX_ANEXOS} arquivos por intimação.` };
  }
  ...
  for (const bruto of brutos) {
    ...
    const nome =
      ...
      path.split("/").pop()!;
    anexos.push({ path, nome, tipo: "ar", content_type: contentType });
  }
  return anexos;
}

export async function registrarIntimacaoEndereco(
  formData: FormData,
  ...
  if (!resultado) return { erro: "Escolha o resultado da intimação." };

  const observacao =
    String(formData.get("observacao") ?? "").trim().slice(0, 500) || null;

  // Registro SEM anexo é aceito (ex.: carta expedida, aguardando o AR) —
  // o resultado é sempre obrigatório.
  ...
  // Limpeza de órfãos (mesmo padrão dos módulos Imóveis e Mandados):
  // qualquer retorno de erro DEPOIS do upload precisa remover o que já
  // subiu — senão o bucket acumula lixo de 20MB a cada tentativa frustrada.
  ...
  for (const a of anexos) {
    ...
    // Anti-replay: path não pode pertencer a outra intimação, a um mandado
    // NEM a uma pesquisa de imóveis — o bucket é compartilhado entre os
    // três módulos.
    ...
    if (donos && donos.length > 0) {
      ...
      await limparAnexos(todosPaths.filter((p) => p !== a.path));
      return { erro: "Este anexo já pertence a outra intimação." };
    }
    const { data: donosMandados, error: donosMandadosErr } = await sb
      .from(TABELA_MANDADOS)
      .select("id")
      .contains("anexos", JSON.stringify([{ path: a.path }]))
      .limit(1);
    if (donosMandadosErr) {
    ...
    if (donosMandados && donosMandados.length > 0) {
      await limparAnexos(todosPaths.filter((p) => p !== a.path));
      return { erro: "Este anexo já pertence a um mandado." };
    }
