...
// Compara o tempo médio (petição de penhora -> penhora efetivada) contra o
// baseline de mercado. Mais rápido que o baseline = verde (up); mais lento =
// vermelho (down). O baseline vem dos dados — a view não chuta valor.
...
export default function TempoMedidaPenhora({ dados }: Props) {
  ...
  // Delta calculado como (baseline - dias): positivo = mais rápido (bom).
  // Sem amostra (zero pares petição/penhora pareados) -> não exibe delta.
  ...
  const subtitulo = semAmostra
    ? `Sem petições pareadas a penhoras efetivadas. Baseline de mercado: ${baseline} dias.`
    : `Da petição de penhora até a penhora efetivada. Baseline de mercado: ${baseline} dias.`;
